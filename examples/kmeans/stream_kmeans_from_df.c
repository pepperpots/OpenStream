/**
 * SHM implementation of K-means, compatible to the non-fuzzy
...
#include "stream_kmeans_from_df_outline.h"
...
int int_pow(int base, int n)
{
	...
	if(n % 2)
		return base * int_pow(base, n-1);

	int t = int_pow(base, n/2);
...
void create_kmeans_consumer_task(struct kmeans_round_params* params, int iter, int block)
{
	...
	#pragma omp task input(sdata_ref[(iter+1) * params->num_blocks + block] >> data_in[1], \
		       sccenter_out_ref[(iter+1) * fan_streams - params->num_blocks + block] >> cc_in[1], \
		       smembership_ref[(iter+1) * params->num_blocks + block] >> mship_in[1], \
...
void create_propagation_cascade_task(struct kmeans_round_params* params, int iter, int level, int num)
{
	...
	#pragma omp task input(sstop_in >> stop_in[1][1], \
			       sccenter_in >> cc_in[1][1]) \
		output(sstop_out << stop_out[params->fan][1],		\
		       sccenter_out << cc_out[params->fan][1])
	{
...
void create_reduction_task(struct kmeans_round_params* params, int iter, int level, int num)
{
	...
	#pragma omp task input(sdelta_in >> delta_in[params->fan][1], \
			       ...
			       sccenter_in >> cc_in[params->fan][1]) \
		output(sdelta_out << delta_out[ndelta_out][1], \
		       snum_members_out << nmemb_out[nnmemb_out][params->k], \
		       sccenter_out << cc_out[1][1], \
		       sstop_out << stop_out[nstop_out][1])
	{
		create_reduction_followup_tasks(params, iter, level, num);

		int nmemb_out_lcl[params->k];
		float delta_out_lcl = delta_in[0][0];
		int base_out = num * params->fan * int_pow(params->fan, params->max_level - level);
		float* cc_out_shm = &params->ccenters_block[base_out*params->k*params->nd];

		for(int j = 0; j < params->k; j++)
			nmemb_out_lcl[j] = nmemb_in[0][j];

		for(int i = 1; i < params->fan; i++) {
			delta_out_lcl += delta_in[i][0];
			int base_in = (num * params->fan + i) * int_pow(params->fan, params->max_level - level);
			float* cc_in_shm = &params->ccenters_block[base_in*params->k*params->nd];

			for(int j = 0; j < params->k; j++) {
				nmemb_out_lcl[j] += nmemb_in[i][j];

				for(int k = 0; k < params->nd; k++)
					cc_out_shm[j*params->nd+k] += cc_in_shm[j*params->nd+k];
			}
		}
		...
		if(level == 0) {
			...
			for(int j = 0; j < params->k; j++)
				for(int k = 0; k < params->nd; k++)
					params->ccenters[j*params->nd+k] = cc_out_shm[j*params->nd+k] / nmemb_out_lcl[j];
...
void create_kmeans_task(struct kmeans_round_params* params, int iter, int block)
{
	...
	#pragma omp task input(sstop_ref[(iter+1) * fan_streams - params->num_blocks + block] >> stop_in[1]) \
		...
		inout_reuse(sdata_ref[iter * params->num_blocks + block] >> data[1] >> sdata_ref[(iter+1) * params->num_blocks + block], \
			    smembership_ref[iter * params->num_blocks + block] >> mship[1] >> smembership_ref[(iter+1) * params->num_blocks + block], \
			    sccenter_in_ref[(iter+1) * fan_streams - params->num_blocks + block] >> cc[1] >> sccenter_out_ref[(iter+1) * fan_streams - params->num_blocks + block])
	{
		float delta;

		if(!stop_in[0]) {
			if(!stop_in[0])
				delta = kmeans(block, params->k, params->nd, params->ccenters, &params->ccenters_block[block*params->k*params->nd], params->block_size, &params->data[block*params->block_size*params->nd], &params->mship[block*params->block_size], &nmemb_out[0]);
			else
				delta = -1.0f;
		} else {
			delta = -1.0f;
...
void create_init_task(struct kmeans_round_params* params, int block)
{
	...
	#pragma omp task output(sstop_ref[fan_streams - params->num_blocks + block] << stop_out[1],	  \
				sdata_ref[block] << data_out[1], \
				sccenter_in_ref[fan_streams - params->num_blocks + block] << cc_out[1], \
				smembership_ref[block] << mship_out[1])
	{
		int stop = 0;
...
int main(int argc, char** argv)
{
	...
	if(!(g_params.ccenters_block = malloc_interleaved(g_params.num_blocks*k_max*g_params.nd*sizeof(float)))) {
		fprintf(stderr, "Could not allocate per-block cluster array.\n");
		exit(1);
	}
