...
#else
static inline void
gauss_seidel (int N, double *a, int block_size)
{
  int i, j;

  for (i = 1; i <= block_size; i++)
    for (j = 1; j <= block_size; j++)
      a[N*i + j] = 0.2 * (a[N*i + j] + a[N*(i-1) + j] + a[N*(i+1) + j] + a[N*i + j-1] + a[N*i + j+1]);
}
#endif
...
main (int argc, char **argv)
{
  ...
  while ((option = getopt(argc, argv, "n:s:b:r:o:h")) != -1)
	...
	case 'h':
	  printf("Usage: %s [option]...\n\n"
		 ...
		 "  -o <output file>             Write data to output file, default is barrier_seidel.out\n",
		 argv[0], N, block_size);
	  exit(0);
	  break;
	case '?':
  ...
  if (res_file == NULL)
    res_file = fopen("barrier_seidel.out", "w");
...
#pragma omp taskwait
    ...
    if (_WITH_OUTPUT)
      {
	printf ("[Barrier] Seidel (size %d, tile %d, iterations %d) executed in %.5f seconds\n",
