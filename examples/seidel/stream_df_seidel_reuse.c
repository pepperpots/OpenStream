/**
 * Dataflow implementation of seidel.c using inout_reuse
 *
...
void gauss_seidel_df_in_place_unrolled(int block_size, double* left, double* top,
...
void create_next_iteration_task(double* matrix, int blocks_x, int blocks_y, int block_size, int numiters, int it, int id_x, int id_y)
{
	...
	double top[block_size];
	double left[block_size];
	double right[block_size];
	double bottom[block_size];
	double center[block_size*block_size];

	/* printf("stop_ref = %p, sbottom_ref = %p, sleft_ref = %p, sright_ref = %p, scenter_ref = %p\n", */
	/*        stop_ref, sbottom_ref, sleft_ref, sright_ref, scenter_ref); */
	...
	if(id_y == 0 && id_x == 0) {
		/* Upper left corner */
		#pragma omp task inout_reuse(stop_ref   [it*blocks + (id+blocks_x)] >> bottom[block_size] >> sbottom_ref[(it+1)*blocks+id],\
					     sleft_ref  [it*blocks + (id+1)       ] >> right[block_size] >> sright_ref [(it+1)*blocks+id], \
					     scenter_ref[it*blocks + id           ] >> center[block_size*block_size] >> scenter_ref[(it+1)*blocks+id])
		{
			gauss_seidel_df_in_place_unrolled(block_size,
						 NULL, NULL, bottom, right, center);
		...
		#pragma omp task inout_reuse(stop_ref   [it*blocks + (id+blocks_x)] >> bottom[block_size] >> sbottom_ref[(it+1)*blocks+id],	\
					     sright_ref [(it+1)*blocks + (id-1)   ] >> left[block_size]   >> sleft_ref  [(it+1)*blocks+id], \
					     sleft_ref  [it*blocks + (id+1)       ] >> right[block_size]  >> sright_ref [(it+1)*blocks+id], \
					     scenter_ref[it*blocks + id           ] >> center[block_size*block_size] >> scenter_ref[(it+1)*blocks+id])
		{
			gauss_seidel_df_in_place_unrolled(block_size,
						 left, NULL, bottom, right, center);
			create_iter_followup_task(matrix, blocks_x, blocks_y, block_size, numiters, it, id_x, id_y);
		}
	} else if(id_y == 0 && id_x == blocks_x-1) {
		/* Upper right corner */
		...
		{
			gauss_seidel_df_in_place_unrolled(block_size,
						 left, NULL, bottom, NULL, center);
			create_iter_followup_task(matrix, blocks_x, blocks_y, block_size, numiters, it, id_x, id_y);
		}
	} else if(id_x == 0 && id_y < blocks_y-1) {
		/* Block in the leftmost column, but not in a corner */
		#pragma omp task inout_reuse(sbottom_ref[(it+1)*blocks + (id-blocks_x)] >> top[block_size] >> stop_ref   [(it+1)*blocks + id], \
					     stop_ref   [it*blocks + (id+blocks_x)] >> bottom[block_size] >> sbottom_ref[(it+1)*blocks + id], \
					     sleft_ref  [it*blocks + (id+1)       ] >> right[block_size] >> sright_ref [(it+1)*blocks + id], \
					     scenter_ref[it*blocks + id           ] >> center[block_size*block_size] >> scenter_ref[(it+1)*blocks + id])
		{
			gauss_seidel_df_in_place_unrolled(block_size,
						 NULL, top, bottom, right, center);
			create_iter_followup_task(matrix, blocks_x, blocks_y, block_size, numiters, it, id_x, id_y);
		}
	} else if(id_x == 0 && id_y == blocks_y-1) {
		/* Lower left corner */
...
int main(int argc, char** argv)
{
	...
	if(res_file == NULL)
		res_file = fopen("stream_df_seidel_reuse.out", "w");
