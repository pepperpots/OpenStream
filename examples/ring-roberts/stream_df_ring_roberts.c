/**
 * Dataflow implementation of the ring-roberts benchmark (ring blur +
 * roberts edge detection), fully compatible to the Polybench version
 * from Polybench.
 *
...
void ring_blur_tile(double* matrix, ssize_t N_y, ssize_t N_x, ssize_t block_size_y, ssize_t block_size_x, ssize_t id_y, ssize_t id_x,
		    double* center_out, double* top_out, double* right_out, double* urcorner_out)
{
	...
	for(ssize_t y = min_y; y <= max_y; y++)
		for(ssize_t x = min_x; x <= max_x; x++)
			center_out[y*block_size_x+x] = GLOBAL_AVG(x, y);

	if(id_x == 0) {
		ssize_t x = 0;
		for(ssize_t y = 0; y < block_size_y; y++)
			center_out[y*block_size_x+x] = GLOBAL_VAL(x, y);
	}

	if(id_x == blocks_x-1) {
		ssize_t x = block_size_x-1;
		for(ssize_t y = 0; y < block_size_y; y++)
			center_out[y*block_size_x+x] = GLOBAL_VAL(x, y);
	}
	...
	if(id_y == 0) {
		ssize_t y = 0;
		for(ssize_t x = 0; x < block_size_x; x++)
			center_out[y*block_size_x+x] = GLOBAL_VAL(x, y);
	}

	if(id_y == blocks_y-1) {
		ssize_t y = block_size_y-1;
		for(ssize_t x = 0; x < block_size_x; x++)
			center_out[y*block_size_x+x] = GLOBAL_VAL(x, y);
	}
	...
	if(right_out) {
		for(ssize_t y = 0; y < block_size_y; y++)
			right_out[y] = center_out[(y+1)*block_size_x-1];
	}

	if(top_out) {
		for(ssize_t x = 0; x < block_size_x; x++)
			top_out[x] = center_out[x];
	}

	if(urcorner_out)
		*urcorner_out = center_out[block_size_x-1];
}

void roberts_edge_tile(double* matrix, ssize_t N_y, ssize_t N_x, ssize_t block_size_y, ssize_t block_size_x, ssize_t id_y, ssize_t id_x,
		    double* center_in, double* left_in, double* bottom_in, double* llcorner_in)
{
	ssize_t blocks_x = N_x / block_size_x;
	ssize_t blocks_y = N_y / block_size_y;
	...
	ssize_t min_x = (id_x == 0) ? 2 : 1;
	ssize_t max_x = (id_x == blocks_x-1) ? block_size_x-2 : block_size_x-1;

	for(ssize_t y = min_y; y <= max_y; y++) {
		for(ssize_t x = min_x; x <= max_x; x++) {
			GLOBAL_VAL(x, y) = (center_in[y*block_size_x+x] -
					    center_in[(y+1)*block_size_x+x-1]) +
					   (center_in[(y+1)*block_size_x+x] -
					    center_in[y*block_size_x+x-1]);
		}
	}

	if(left_in) {
		ssize_t x = 0;
		for(ssize_t y = min_y; y <= min(max_y, block_size_y-2); y++) {
			GLOBAL_VAL(x, y) = (center_in[y*block_size_x+x] -
					    left_in[y+1]) +
					   (center_in[(y+1)*block_size_x+x] -
					    left_in[y]);
	...
	if(bottom_in) {
		ssize_t y = block_size_y-1;
		for(ssize_t x = max(min_x, 1); x <= max_x; x++) {
			GLOBAL_VAL(x, y) = (center_in[y*block_size_x+x] -
					    bottom_in[x-1]) +
					   (bottom_in[x] -
					    center_in[y*block_size_x+x-1]);
...
void create_ring_task(double* matrix, ssize_t N_y, ssize_t N_x, ssize_t block_size_y, ssize_t block_size_x, ssize_t id_y, ssize_t id_x)
{
	/* Arrays used to access the data from the streams */
	double top_out[block_size_x];
	double right_out[block_size_y];
	double center_out[block_size_x*block_size_y];
	...
	if(id_y == 0 && id_x < blocks_x-1) {
		...
		#pragma omp task output(sright_ref [id] << right_out[block_size_y], \
					scenter_ref[id] << center_out[block_size_x*block_size_y], \
					writeauths_refs << writeauths_out[nwriteauths][1])
		{
			ring_blur_tile(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x,
				       center_out, NULL, right_out, NULL);
		}
	} else if(id_y == 0 && id_x == blocks_x-1) {
		/* Upper right */
		#pragma omp task output(scenter_ref[id] << center_out[block_size_x*block_size_y], \
					writeauths_refs << writeauths_out[nwriteauths][1])
		{
			ring_blur_tile(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x,
				       center_out, NULL, NULL, NULL);
		}
	} else if(id_y > 0 && id_x < blocks_x-1) {
		/* Not in the right column, not in the top row */
		#pragma omp task output(sright_ref [id] << right_out[block_size_y], \
					stop_ref [id] << top_out[block_size_x], \
					scenter_ref[id] << center_out[block_size_x*block_size_y], \
					surcorner_ref[id] << urcorner_out, \
					writeauths_refs << writeauths_out[nwriteauths][1])
		{
			ring_blur_tile(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x,
				       center_out, top_out, right_out, &urcorner_out);
		}
	} else if (id_y > 0 && id_x == blocks_x-1) {
		/* right column, not in the top row */
		#pragma omp task output(stop_ref [id] << top_out[block_size_x], \
					scenter_ref[id] << center_out[block_size_x*block_size_y], \
					writeauths_refs << writeauths_out[nwriteauths][1])
		{
			ring_blur_tile(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x,
				       center_out, top_out, NULL, NULL);
		}
	}
}

void create_roberts_task(double* matrix, ssize_t N_y, ssize_t N_x, ssize_t block_size_y, ssize_t block_size_x, ssize_t id_y, ssize_t id_x)
{
	/* Arrays used to access the data from the streams */
	double bottom_in[block_size_x];
	double left_in[block_size_y];
	double center_in[block_size_x*block_size_y];
	...
	if(id_y == blocks_y-1 && id_x == 0) {
		/* Lower left corner */
		#pragma omp task input(scenter_ref[id] >> center_in[block_size_x*block_size_y], \
				       swriteauth_ref[id] >> writeauth_tokens[nwriteauth_tokens]) \
			output(sdfbarrier_ref[0] << barrier_token)
		{
			roberts_edge_tile(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x,
					  center_in, NULL, NULL, NULL);
		}
	} else if(id_y == blocks_y-1 && id_x > 0) {
		/* Lower row, not in lower left corner  */
		#pragma omp task input(scenter_ref[id] >> center_in[block_size_x*block_size_y], \
				       sright_ref[id-1] >> left_in[block_size_y], \
				       swriteauth_ref[id] >> writeauth_tokens[nwriteauth_tokens]) \
			output(sdfbarrier_ref[0] << barrier_token)
		{
			roberts_edge_tile(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x,
					  center_in, left_in, NULL, NULL);
		}
	} else if(id_y < blocks_y-1 && id_x == 0) {
		/* left column */
		#pragma omp task input(scenter_ref[id] >> center_in[block_size_x*block_size_y], \
				       stop_ref[id+blocks_x] >> bottom_in[block_size_x], \
				       swriteauth_ref[id] >> writeauth_tokens[nwriteauth_tokens]) \
			output(sdfbarrier_ref[0] << barrier_token)
		{
			roberts_edge_tile(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x,
					  center_in, NULL, bottom_in, NULL);
		}
		...
		{
			roberts_edge_tile(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x,
					  center_in, left_in, bottom_in, &llcorner_in);
		}
	}
}
			...
			#pragma omp task
			{
				for(ssize_t id_y = id_yy; id_y < min(id_yy + tasks_per_block_y, blocks_y); id_y++) {
					for(ssize_t id_x = id_xx; id_x < min(id_xx + tasks_per_block_x, blocks_x); id_x++) {
						create_ring_task(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x);
						create_roberts_task(matrix, N_y, N_x, block_size_y, block_size_x, id_y, id_x);
					}
				}
			}
		}
