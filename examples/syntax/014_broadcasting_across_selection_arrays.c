...
#pragma omp tick (streams[2] >> 4)
	  /* Alternatively, it is possible to tick the streams in the
	     original array "x" (the permutation used does not impact
	     this as all streams are used):
#pragma omp tick (x[0] >> 4)
#pragma omp tick (x[1] >> 4)
#pragma omp tick (x[2] >> 4)
	  */
	}
