...
def dump_main_fun(config):
    ...
    sys.stdout.write("				       \"  -o <output file>             Write data to output file, default is jacobi_%dd.out\\n\",\n"%config["num_dims"])
    ...
    sys.stdout.write("		res_file = fopen(\"jacobi_%dd.out\", \"w\");\n"%config["num_dims"])
    sys.stdout.write("\n")
    ...
    for dim in range(config["num_dims"]):
        sys.stdout.write(indent+"int "+otherdim_prefix(dim, config["dim_names"])+"_saver_size = sizeof(double)")
    ...
    for dim in range(config["num_dims"]):
            sys.stdout.write("\tdouble* "+otherdim_prefix(dim, config["dim_names"])+"_saver = malloc("+otherdim_prefix(dim, config["dim_names"])+"_saver_size);\n")

    sys.stdout.write("\n")

    for dim in range(config["num_dims"]):
            sys.stdout.write("\tmemset("+otherdim_prefix(dim, config["dim_names"])+"_saver, 0, "+otherdim_prefix(dim, config["dim_names"])+"_saver_size);\n")
    ...
    for dim in range(config["num_dims"]):
        sys.stdout.write(indent+"\tdouble "+config["dimref_names"][dim][0]+"_val = ("+config["dim_names"][dim]+" == 1) ? matrix[index_"+config["dimref_names"][dim][0]+"] : "+otherdim_prefix(dim, config["dim_names"])+"_saver["+otherdim_prefix(dim, config["dim_names"])+"_saver_index];\n")
    ...
    sys.stdout.write("\n")
    sys.stdout.write(indent+"\tmatrix[index_center] = new_val;\n")
    ...
    for dim in range(config["num_dims"]):
        sys.stdout.write("\tfree("+otherdim_prefix(dim, config["dim_names"])+"_saver);\n")
