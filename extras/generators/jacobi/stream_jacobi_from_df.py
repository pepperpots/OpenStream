...
def dump_terminal_task(config, pref):
    ...
    sys.stdout.write("scenter_ref[id] >> center_in[1]")

    for dim in range(config["num_dims"]):
        if config["dim_configs"][dim] != jacobi_positions.first and config["dim_configs"][dim] != jacobi_positions.span:
            sys.stdout.write(", \\\n"+pref+"\t\t")
            sys.stdout.write("s"+config["dimref_names"][dim][0]+"_ref[id] >> "+config["dimref_names"][dim][0]+"_in[1]")
            num_clauses = num_clauses + 1

        if config["dim_configs"][dim] != jacobi_positions.last and config["dim_configs"][dim] != jacobi_positions.span:
            sys.stdout.write(", \\\n"+pref+"\t\t")
            sys.stdout.write("s"+config["dimref_names"][dim][1]+"_ref[id] >> "+config["dimref_names"][dim][1]+"_in[1]")
            num_clauses = num_clauses + 1
...
def dump_create_terminal_task_fun(config):
    ...
    sys.stdout.write("\tdouble center_in[1];\n")
    ...
    for dim in range(config["num_dims"]):
        ...
        sys.stdout.write("\tdouble "+config["dimref_names"][dim][0]+"_in[1];\n")
        sys.stdout.write("\tdouble "+config["dimref_names"][dim][1]+"_in[1];\n")
...
def dump_init_task(config, pref):
    ...
    sys.stdout.write("scenter_ref[id] << center_out[1]")

    for dim in range(config["num_dims"]):
        if config["dim_configs"][dim] != jacobi_positions.first and config["dim_configs"][dim] != jacobi_positions.span:
            sys.stdout.write(", \\\n"+pref+"\t\t")
            sys.stdout.write("s"+config["dimref_names"][dim][0]+"_ref[id] << "+config["dimref_names"][dim][0]+"_out[1]")
            num_clauses = num_clauses + 1

        if config["dim_configs"][dim] != jacobi_positions.last and config["dim_configs"][dim] != jacobi_positions.span:
            sys.stdout.write(", \\\n"+pref+"\t\t")
            sys.stdout.write("s"+config["dimref_names"][dim][1]+"_ref[id] << "+config["dimref_names"][dim][1]+"_out[1]")
            num_clauses = num_clauses + 1
    ...
    for dim in range(config["num_dims"]):
        if config["dim_configs"][dim] != jacobi_positions.first and config["dim_configs"][dim] != jacobi_positions.span:
            sys.stdout.write(", global_savers_"+config["dimref_names"][dim][0])
        else:
            sys.stdout.write(", NULL")

        if config["dim_configs"][dim] != jacobi_positions.last and config["dim_configs"][dim] != jacobi_positions.span:
            sys.stdout.write(", global_savers_"+config["dimref_names"][dim][1])
        else:
            sys.stdout.write(", NULL")
    ...
    for dim in range(config["num_dims"]):
        for direction in range(2):
            sys.stdout.write(", global_savers_"+config["dimref_names"][dim][direction])
...
def dump_create_initial_task_fun_signature(config):
    ...
    for dim in range(config["num_dims"]):
        for direction in range(2):
            sys.stdout.write(", double* global_savers_"+config["dimref_names"][dim][direction])
...
def dump_create_initial_task_fun(config):
    ...
    sys.stdout.write("\tdouble center_out[1];\n")
    ...
    for dim in range(config["num_dims"]):
        ...
        sys.stdout.write("\tdouble "+config["dimref_names"][dim][0]+"_out[1];\n")
        sys.stdout.write("\tdouble "+config["dimref_names"][dim][1]+"_out[1];\n")
...
def dump_init_fun(config):
    ...
    for dim in range(config["num_dims"]):
        for direction in range(2):
            sys.stdout.write("	size_t one_saver_size_"+config["dimref_names"][dim][direction]+" = 1")
            for dim_other in range(config["num_dims"]):
                if dim_other != dim:
    ...
    for dim in range(config["num_dims"]):
        sys.stdout.write("	double* "+config["dimref_names"][dim][0]+"_out = (id_"+config["dim_names"][dim]+" == 0) ? NULL : &global_savers_"+config["dimref_names"][dim][0]+"[id*one_saver_size_"+config["dimref_names"][dim][0]+"];\n")
        sys.stdout.write("	double* "+config["dimref_names"][dim][1]+"_out = (id_"+config["dim_names"][dim]+" == blocks_"+config["dim_names"][dim]+"-1) ? NULL : &global_savers_"+config["dimref_names"][dim][1]+"[id*one_saver_size_"+config["dimref_names"][dim][1]+"];\n")
...
def dump_next_iteration_task(config, pref):
    ...
    num_clauses = 0
    sys.stdout.write(pref+"\tinput(")
    sys.stdout.write("scenter_ref[id_center] >> center_in[1]")

    for dim in range(config["num_dims"]):
        if config["dim_configs"][dim] != jacobi_positions.first and config["dim_configs"][dim] != jacobi_positions.span:
            sys.stdout.write(", \\\n"+pref+"\t\t")
            sys.stdout.write("s"+config["dimref_names"][dim][1]+"_ref[id_"+config["dimref_names"][dim][0]+"] >> "+config["dimref_names"][dim][0]+"_in[1]")
            num_clauses = num_clauses + 1

        if config["dim_configs"][dim] != jacobi_positions.last and config["dim_configs"][dim] != jacobi_positions.span:
            sys.stdout.write(", \\\n"+pref+"\t\t")
            sys.stdout.write("s"+config["dimref_names"][dim][0]+"_ref[id_"+config["dimref_names"][dim][1]+"] >> "+config["dimref_names"][dim][1]+"_in[1]")
            num_clauses = num_clauses + 1
    ...
    sys.stdout.write(pref+"\tjacobi_%dd_df_in_place(matrix, it"%config["num_dims"])
...
def dump_jacobi_fun_signature(config):
    sys.stdout.write("void jacobi_%dd_df_in_place(double* matrix, int it"%config["num_dims"])

    for dim in range(config["num_dims"]):
        sys.stdout.write(", int blocks_"+config["dim_names"][dim])
...
def dump_jacobi_fun(config):
    ...
    for dim in range(config["num_dims"]):
        sys.stdout.write("\tint N_"+config["dim_names"][dim]+" = block_size_"+config["dim_names"][dim]+" * blocks_"+config["dim_names"][dim]+";\n")
    ...
    for dim in range(config["num_dims"]):
        for direction in range(2):
            sys.stdout.write("	size_t one_saver_elems_"+config["dimref_names"][dim][direction]+" = 1")
            for dim_other in range(config["num_dims"]):
                if dim_other != dim:
                    sys.stdout.write("*block_size_"+config["dim_names"][dim_other])
            sys.stdout.write(";\n")
    ...
    for dim in range(config["num_dims"]):
        sys.stdout.write("\tint "+config["dim_names"][dim]+"_dir = (id_"+config["dim_names"][dim]+" != 0) ? 1 : -1;\n")
    ...
    for dim in range(config["num_dims"]):
        sys.stdout.write("\tdouble* "+config["dimref_names"][dim][0]+" = &global_savers_"+config["dimref_names"][dim][1]+"[((it % 2)*blocks + id_"+config["dimref_names"][dim][0]+")*one_saver_elems_"+config["dimref_names"][dim][1]+"];\n")
        sys.stdout.write("\tdouble* "+config["dimref_names"][dim][1]+" = &global_savers_"+config["dimref_names"][dim][0]+"[((it % 2)*blocks + id_"+config["dimref_names"][dim][1]+")*one_saver_elems_"+config["dimref_names"][dim][0]+"];\n")

    sys.stdout.write("\n")

    for dim in range(config["num_dims"]):
        sys.stdout.write("\tdouble* "+config["dimref_names"][dim][0]+"_out = &global_savers_"+config["dimref_names"][dim][0]+"[(((it+1) % 2)*blocks + id)*one_saver_elems_"+config["dimref_names"][dim][0]+"];\n")
        sys.stdout.write("\tdouble* "+config["dimref_names"][dim][1]+"_out = &global_savers_"+config["dimref_names"][dim][1]+"[(((it+1) % 2)*blocks + id)*one_saver_elems_"+config["dimref_names"][dim][1]+"];\n")

    sys.stdout.write("\n")

    for dim in range(config["num_dims"]):
        sys.stdout.write("\tdouble* "+otherdim_prefix(dim, config["dim_names"])+"_saver = ("+config["dim_names"][dim]+"_dir == 1) ? "+config["dimref_names"][dim][0]+"_out : "+config["dimref_names"][dim][1]+"_out;\n")
    ...
    for dim in range(config["num_dims"]):
        for direction in range(0, 2):
            ...
            if direction == 0:
                sys.stdout.write("("+config["dim_names"][dim]+" == 0) ?\n"+indent+"\t(id_"+config["dim_names"][dim]+" != 0 ? "+config["dimref_names"][dim][direction]+"[index_"+config["dimref_names"][dim][direction]+"] : 0.0) :\n"+indent+"\t(("+config["dim_names"][dim]+"_dir == 1) ?\n"+indent+"\t\t"+otherdim_prefix(dim, config["dim_names"])+"_saver["+otherdim_prefix(dim, config["dim_names"])+"_saver_index] :\n"+indent+"\t\tmatrix[global_index_"+config["dimref_names"][dim][direction]+"])")
            else:
                sys.stdout.write("("+config["dim_names"][dim]+" == block_size_"+config["dim_names"][dim]+"-1) ?\n"+indent+"\t(id_"+config["dim_names"][dim]+" != blocks_"+config["dim_names"][dim]+"-1 ? "+config["dimref_names"][dim][direction]+"[index_"+config["dimref_names"][dim][direction]+"] : 0.0) :\n"+indent+"\t(("+config["dim_names"][dim]+"_dir == 1) ?\n"+indent+"\t\tmatrix[global_index_"+config["dimref_names"][dim][direction]+"] :\n"+indent+"\t\t"+otherdim_prefix(dim, config["dim_names"])+"_saver["+otherdim_prefix(dim, config["dim_names"])+"_saver_index])")
            sys.stdout.write(";\n");
    ...
    sys.stdout.write(indent+"matrix[global_index_center] = new_val;\n")
    ...
    for dim in range(config["num_dims"]):
        ...
        for dim_other in range(config["num_dims"]):
            if dim_other != dim:
                sys.stdout.write(indent+"int global_"+config["dim_names"][dim_other]+" = id_"+config["dim_names"][dim_other]+"*block_size_"+config["dim_names"][dim_other]+" + "+config["dim_names"][dim_other]+";\n")
        ...
        for direction in range(2):
            sys.stdout.write("\n")
            sys.stdout.write(indent+"if("+config["dimref_names"][dim][direction]+"_out) {\n")
            if direction == 0:
                sys.stdout.write(indent+"\tint global_"+config["dim_names"][dim]+"_"+config["dimref_names"][dim][direction]+" = id_"+config["dim_names"][dim]+"*block_size_"+config["dim_names"][dim]+";\n")
            else:
                sys.stdout.write(indent+"\tint global_"+config["dim_names"][dim]+"_"+config["dimref_names"][dim][direction]+" = (id_"+config["dim_names"][dim]+"+1)*block_size_"+config["dim_names"][dim]+" - 1;\n")

            sys.stdout.write(indent+"\tint global_index_src = 0")
            for dim_other in range(config["num_dims"]):
                if dim_other == dim:
                    sys.stdout.write(" + global_"+config["dim_names"][dim_other]+"_"+config["dimref_names"][dim][direction])
                else:
                    sys.stdout.write(" + global_"+config["dim_names"][dim_other]+"")
            ...
            sys.stdout.write(indent+"\t"+config["dimref_names"][dim][direction]+"_out["+otherdim_prefix(dim, config["dim_names"])+"_saver_index] = matrix[global_index_src];\n")
...
def dump_main_fun(config):
    ...
    sys.stdout.write("				       \"  -o <output file>             Write data to output file, default is stream_jacobi_%dd_from_df.out\\n\",\n"%config["num_dims"])
    ...
    for dim in range(config["num_dims"]):
        for direction in range(2):
            sys.stdout.write("	size_t one_saver_size_"+config["dimref_names"][dim][direction]+" = sizeof(double)")
            for dim_other in range(config["num_dims"]):
                if dim_other != dim:
                    sys.stdout.write("*block_size_"+config["dim_names"][dim_other])
            sys.stdout.write(";\n")
    ...
    sys.stdout.write("		res_file = fopen(\"stream_jacobi_%dd_from_df.out\", \"w\");\n"%config["num_dims"])
    sys.stdout.write("\n")
    ...
    for dim in range(config["num_dims"]):
        for direction in range(2):
                sys.stdout.write("	free(global_savers_"+config["dimref_names"][dim][direction]+");\n")
