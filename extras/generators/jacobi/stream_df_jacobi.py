...
def dump_next_iteration_task(config, pref):
    ...
    sys.stdout.write(pref+"\tjacobi_%dd_df("%config["num_dims"])
...
def dump_jacobi_fun_signature(config):
    sys.stdout.write("void jacobi_%dd_df("%config["num_dims"])
...
def dump_jacobi_fun(config):
    ...
    for dim in range(config["num_dims"]):
        for direction in range(0, 2):
            ...
            if direction == 0:
                sys.stdout.write("("+config["dim_names"][dim]+" == 0) ? ("+config["dimref_names"][dim][direction]+"_in ? "+config["dimref_names"][dim][direction]+"_in[index_"+config["dimref_names"][dim][direction]+"] : 0.0) : center_in[index_"+config["dimref_names"][dim][direction]+"_center]")
            else:
    ...
    sys.stdout.write(indent+"center_out[index_center] = new_val;\n")

    for dim in range(config["num_dims"]):
        sys.stdout.write("\n"+indent+"if("+config["dim_names"][dim]+" == 0 && "+config["dimref_names"][dim][0]+"_out)\n")
        sys.stdout.write(indent+"\t"+config["dimref_names"][dim][0]+"_out[index_"+config["dimref_names"][dim][0]+"] = new_val;\n")

        sys.stdout.write("\n"+indent+"if("+config["dim_names"][dim]+" == block_size_"+config["dim_names"][dim]+"-1 && "+config["dimref_names"][dim][1]+"_out)\n")
        sys.stdout.write(indent+"\t"+config["dimref_names"][dim][1]+"_out[index_"+config["dimref_names"][dim][1]+"] = new_val;\n")
...
def dump_main_fun(config):
    ...
    sys.stdout.write("				       \"  -o <output file>             Write data to output file, default is stream_df_jacobi_%dd.out\\n\",\n"%config["num_dims"])
    ...
    sys.stdout.write("		res_file = fopen(\"stream_df_jacobi_%dd.out\", \"w\");\n"%config["num_dims"])
    sys.stdout.write("\n")
